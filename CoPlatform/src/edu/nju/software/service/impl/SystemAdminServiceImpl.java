...
public class SystemAdminServiceImpl implements SystemAdminService {
	private static Logger logger = LoggerFactory.getLogger(SystemAdminServiceImpl.class);
	...
	public GeneralResult<List<SystemAdmin>> getAll() {
		...
		List<SystemAdmin> systemAdminList = (List<SystemAdmin>) CoCacheManager.get(ALL_SYSTEM_ADMIN_CACHE_KEY);
		if(null != systemAdminList && !systemAdminList.isEmpty()) {
			...
			try {
				...
				if(null != systemAdminList && !systemAdminList.isEmpty()) {
					result.setData(systemAdminList);
					CoCacheManager.put(ALL_SYSTEM_ADMIN_CACHE_KEY, systemAdminList);
				}else {
